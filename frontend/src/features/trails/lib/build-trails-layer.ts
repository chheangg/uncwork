...
export const buildTrailsLayers = (
  ...
  const dashedHistoryProps = {
    ...
    getColor: (d: TrackPath) => perVertexColor(d, 240, fadeNow),
    ...
    // Tighter dash (more painted, less gap) so the trail reads as a
    // continuous line on busy basemaps instead of dotty fragments.
    getDashArray: [10, 4] as [number, number],
    dashJustified: false,
    updateTriggers: {
      getColor: fadeNow,
    },
